...
class DeliverymanController extends Controller
{
    ...
    public function get_profile(Request $request)
    {
        ...
        $dm['order_count'] =(integer)$dm->orders->where("canceled" , null)->count();
        $dm['todays_order_count'] =(integer)$dm->todaysorders->where("canceled" , null)->count();
        $dm['this_week_order_count'] =(integer)$dm->this_week_orders->where("canceled" , null)->count();
        ...
        unset($dm['wallet']);
        // print_r(json_encode( $dm->orders->where("canceled" , null) ));die;
    ...
    public function get_latest_orders(Request $request)
    {
        ...
        {
            $orders = $orders->whereIn('order_status', ['pending', 'accepted','confirmed','processing','handover']);
        }
        else
        {
            $orders = $orders->whereIn('order_status', ['pending','accepted','confirmed','processing','handover']);
    ...
    public function accept_order(Request $request)
    {
        ...
        $order->order_status = in_array($order->order_status, ['pending', 'confirmed'])?'handover':$order->order_status;
    ...
    public function update_order_status(Request $request)
    {
        ...
        if ($request->status == 'delivered') 
        {
            if($order->transaction == null)
            {
                ...
                // try{
                //     $status = OrderLogic::create_transaction($order,$reveived_by, null);
