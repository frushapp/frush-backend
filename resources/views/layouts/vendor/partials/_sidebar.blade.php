    ...
    <aside
        ...
        <div class="navbar-vertical-container">
            <div class="navbar-brand-wrapper justify-content-center"
                onclick="location.href='{{ route('vendor.dashboard') }}'"
            ...
            <div class="navbar-vertical-content text-capitalize" style="background-color: #334257;">
                <ul class="navbar-nav navbar-nav-lg nav-tabs">
                    ...
                    @if (\App\CentralLogics\Helpers::employee_module_permission_check('order'))
                        <li class="nav-item">
                            <small class="nav-subtitle"
                                title="{{ __('messages.order') }} {{ __('messages.section') }}">{{ __('messages.order') }}
                            ...
                            <ul class="js-navbar-vertical-aside-submenu nav nav-sub"
                                ...
                                <li class="nav-item {{ Request::is('vendor-panel/order/list/pending') ? 'active' : '' }}">
                                    <a class="nav-link " href="{{ route('vendor.order.list', ['pending']) }}"
                                        title="{{ __('messages.pending') }}({{ __('messages.take_away') }})">
                                        <span class="tio-circle nav-indicator-icon"></span>
                                        <span class="text-truncate">
                                            {{ __('messages.pending') }}
                                            {{ config('order_confirmation_model') == 'restaurant' || \App\CentralLogics\Helpers::get_restaurant_data()->self_delivery_system ? '' : __('messages.take_away') }}
                                ...
                                <li
                                    class="nav-item {{ Request::is('vendor-panel/order/list/food_on_the_way') ? 'active' : '' }}">
                                    <a class="nav-link" href="{{ route('vendor.order.list', ['food_on_the_way']) }}"
                                        title="{{ __('messages.foods_on_the_way') }}">
                                        <span class="tio-circle nav-indicator-icon"></span>
                                        <span class="text-truncate">
                                        ...
                                        <span class="text-truncate">
                                            ...
                                            <span class="badge badge-info badge-pill ml-1">
                                                {{ \App\Models\Order::where('restaurant_id', \App\CentralLogics\Helpers::get_restaurant_id())->Notpos()->Scheduled()->where(function ($q) {
                                                        if (
                                                            config('order_confirmation_model') == 'restaurant' ||
                                                            \App\CentralLogics\Helpers::get_restaurant_data()->self_delivery_system
                                                        ) {
                                ...
                                <li class="nav-item {{ Request::is('vendor-panel/order/list/all') ? 'active' : '' }}">
                                    <a class="nav-link" href="{{ route('vendor.order.list', ['all']) }}"
                                        title="{{ __('messages.all') }} {{ __('messages.order') }}">
                                        <span class="tio-circle nav-indicator-icon"></span>
                                        <span class="text-truncate">
                                            {{ __('messages.all') }}
                                            <span class="badge badge-info badge-pill ml-1">
                                                {{ \App\Models\Order::where('restaurant_id', \App\CentralLogics\Helpers::get_restaurant_id())->where(function ($query) {
                    ...
                    @if (\App\CentralLogics\Helpers::employee_module_permission_check('pos'))
                        <li class="nav-item">
                            <small class="nav-subtitle">{{ __('messages.pos') }} {{ __('messages.system') }}</small>
                            <small class="tio-more-horizontal nav-subtitle-replacer"></small>
                        </li>
                        ...
                        <li
                            ...
                            <ul class="js-navbar-vertical-aside-submenu nav nav-sub"
                                ...
                                <li class="nav-item {{ Request::is('vendor-panel/pos/orders') ? 'active' : '' }}">
                                    <a class="nav-link " href="{{ route('vendor.pos.orders') }}"
                                        title="{{ __('messages.orders') }}">
                                        <span class="tio-circle nav-indicator-icon"></span>
                    ...
                    @endif
                    <li class="nav-item">
                        <small class="nav-subtitle">{{ __('messages.food') }} {{ __('messages.management') }}</small>
                        <small class="tio-more-horizontal nav-subtitle-replacer"></small>
                    </li>
                    ...
                    @if (\App\CentralLogics\Helpers::employee_module_permission_check('food'))
                        <!-- Food -->
                        <li
                            class="navbar-vertical-aside-has-menu {{ Request::is('vendor-panel/food*') ? 'active' : '' }}">
                            <a class="js-navbar-vertical-aside-menu-link nav-link nav-link-toggle" href="javascript:">
                            ...
                            <ul class="js-navbar-vertical-aside-submenu nav nav-sub"
                                ...
                                @if (\App\CentralLogics\Helpers::get_restaurant_data()->food_section)
                                    <li
                                        class="nav-item {{ Request::is('vendor-panel/food/bulk-import') ? 'active' : '' }}">
                                    ...
                                    <li
                                        class="nav-item {{ Request::is('vendor-panel/food/bulk-export') ? 'active' : '' }}">
                    ...
                    <li class="nav-item">
                        <small class="nav-subtitle">{{ __('messages.marketing') }}
                            {{ __('messages.section') }}</small>
                        <small class="tio-more-horizontal nav-subtitle-replacer"></small>
                    </li>
                    <!-- Campaign -->
                    ...
                    @if (\App\CentralLogics\Helpers::employee_module_permission_check('restaurant_setup'))
                        <li
                            class="nav-item {{ Request::is('vendor-panel/business-settings/restaurant-setup') ? 'active' : '' }}">
                            <a class="nav-link " href="{{ route('vendor.business-settings.restaurant-setup') }}"
                                title="{{ __('messages.restaurant') }} {{ __('messages.config') }}">
                                <span class="tio-settings nav-icon"></span>
                                <span class="text-truncate">{{ __('messages.restaurant') }}
                    ...
                    @if (\App\CentralLogics\Helpers::employee_module_permission_check('my_shop'))
                        <li
                            class="navbar-vertical-aside-has-menu {{ Request::is('vendor/restaurant/*') ? 'active' : '' }}">
                            <a class="js-navbar-vertical-aside-menu-link nav-link"
                                href="{{ route('vendor.shop.view') }}" title="{{ __('messages.my_shop') }}">
                    ...
                    @if (\App\CentralLogics\Helpers::employee_module_permission_check('wallet'))
                        ...
                        <li class="navbar-vertical-aside-has-menu {{ Request::is('vendor/wallet*') ? 'active' : '' }}">
                            <a class="js-navbar-vertical-aside-menu-link nav-link"
                                ...
                                title="{{ __('messages.my') }} {{ __('messages.wallet') }}">
                                <i class="tio-table nav-icon"></i>
                                <span
                                    class="navbar-vertical-aside-mini-mode-hidden-elements text-truncate">{{ __('messages.my') }}
                                    {{ __('messages.wallet') }}</span>
                            </a>
                        </li>
                    @endif
                    ...
                    <li class="nav-item">
                        <small class="nav-subtitle"
                            title="{{ __('messages.employee') }} {{ __('messages.section') }}">{{ __('messages.employee') }}
                            {{ __('messages.section') }}</small>
                        <small class="tio-more-horizontal nav-subtitle-replacer"></small>
                    </li>
