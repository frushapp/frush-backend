        ...
        <div class="col-lg-12 pt-3">
            <form action="{{route('admin.report.set-date')}}" method="post">
                ...
                <div class="row">
                    ...
                    <div class="col-4">
                        <div class="mb-3">
                            <select name="status" id="status" class="form-control" {{session()->has('status')?'value='.session('status'):''}}>
                ...
                <tbody id="set-rows">
                    ...
                    <?php foreach ($foods as $row) { ?>
                        ...
                        <?php if (!empty($row->add_ons)) {
                            ...
                            // Step 1: Clean the input string by wrapping it in square brackets to form a valid JSON array
                            $cleaned_data = '[' . implode(',', array_filter(explode('],', $data))) . ']';

                            // Step 2: Decode the JSON string into a PHP array
                            $decoded_data = json_decode($cleaned_data, true);

                            // Step 3: Process the decoded data
                            echo "<pre>";
...
<script>
    ...
    $('#search-form').on('submit', function(e) {
        ...
        $.post({
            url: '{{route('
            admin.report.food - wise - report - search ')}}',
            data: formData,
            cache: false,
            contentType: false,
            processData: false,
    ...
    $(document).ready(function() {
        $('#datatable').DataTable({
            dom: 'Bfrtip', // 'B' indicates the export buttons
            buttons: [
                'csv', 'excel', 'pdf'
            ]
        });
