                                        ...
                                        @if (count(json_decode($detail['variation'], true)) > 0)
                                            ...
                                            @foreach (json_decode($detail['variation'], true)[0] as $key1 => $variation)
                                                <div class="font-size-sm text-body">
                                                    <span>{{ $key1 }} : </span>
                                                    <strong
                                                        class="font-weight-bold">{{ $key1 == 'price' ? \App\CentralLogics\Helpers::format_currency($variation) : $variation }}</strong>
                                                </div>
                                            @endforeach
                                        @else
                                            <div class="font-size-sm text-body">
                                                <span>{{ 'Price' }} : </span>
                                                <strong
                                                    class="font-weight-bold">{{ \App\CentralLogics\Helpers::format_currency($detail->price) }}</strong>
                                            </div>
                                        @endif
